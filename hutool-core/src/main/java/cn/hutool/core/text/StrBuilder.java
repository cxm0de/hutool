...
public class StrBuilder implements CharSequence, Appendable, Serializable {
    ...
    public StrBuilder del(int start, int end) {
        ...
        // 操作步骤：迁移字符，设定结束位置。这里的代码有重复计算的嫌疑。
        // 这里与下面的this.position -= len;在删除最后一个字符时会重复计算
        // 这一句完全是多此一举。
//        if (end == this.position) {
//            this.position = start;
//        }
        ...
        int len = end - start;
        if (len > 0) {
            // 原代码最后长度部分有问题由【this.position - end】 改为 【this.position - end + 1】
            System.arraycopy(value, start + len, value, start, this.position - end + 1);
            this.position -= len;
        }
        return this;
    }
